...
export default function Chatbot() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.success ? data.reply : "Sorry, couldn't process that." }])
  ...
  return (
    <>
      ...
      {open && (
        <div style={{position:'fixed',bottom:90,right:24,width:360,maxHeight:560,background:'var(--card)',border:'1px solid var(--b2)',borderRadius:18,display:'flex',flexDirection:'column',zIndex:8000,boxShadow:'0 24px 64px rgba(0,0,0,.6)',animation:'chatIn .25s ease',overflow:'hidden'}}>
          <div style={{padding:'16px 18px',background:'var(--bg2)',borderBottom:'1px solid var(--b1)',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,color:'var(--t1)'}}>Sapan's Assistant</div>
            ...
            <button onClick={()=>setOpen(false)} style={{width:28,height:28,borderRadius:7,background:'rgba(255,255,255,.06)',border:'1px solid var(--b2)',color:'var(--t3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'16px 16px 8px',display:'flex',flexDirection:'column',gap:12,minHeight:0}}>
            {messages.map((m,i)=>(
              <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                {m.role==='user'?<div className="chat-msg-user">{m.content}</div>:<div className="chat-msg-bot">{m.content}</div>}
              </div>
            ))}
            {loading&&<div style={{display:'flex',justifyContent:'flex-start'}}><TypingDots/></div>}
            {messages.length===1&&!loading&&(
              <div style={{display:'flex',flexWrap:'wrap',gap:7,marginTop:4}}>
                {SUGGESTED.map(q=><button key={q} className="chat-suggest" onClick={()=>send(q)}>{q}</button>)}
              </div>
            )}
            <div ref={bottomRef}/>
          </div>
          ...
          <div style={{padding:'12px 14px',borderTop:'1px solid var(--b1)',background:'var(--bg2)',flexShrink:0}}>
            <div style={{display:'flex',alignItems:'center',gap:10,background:'var(--bg3)',border:'1.5px solid var(--b2)',borderRadius:10,padding:'8px 10px 8px 14px'}}>
              <input ref={inputRef} className="chat-input" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKey} placeholder="Ask about skills, projects..." disabled={loading} maxLength={300}/>
      ...
      <button onClick={()=>setOpen(o=>!o)}
        ...
        {unread>0&&!open&&<div style={{position:'absolute',top:-2,right:-2,width:18,height:18,borderRadius:'50%',background:'#fb7185',border:'2px solid var(--bg)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,color:'#fff'}}>{unread}</div>}
      </button>
    </>
  )
}
